...
namespace XLEngine
{
	...
	void OpenGLFramebuffer::Refresh()
	{
		...
		{
			XL_CORE_ASSERT(m_ColorAttachments.size() <= 4, "m_ColorAttachments.size() <= 4");
	...
	{
		XL_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size(),"attachmentIndex < m_ColorAttachments.size()");
