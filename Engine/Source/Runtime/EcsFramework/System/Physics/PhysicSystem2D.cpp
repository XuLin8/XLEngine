...
namespace XLEngine
{
	...
	void XLEngine::PhysicSystem2D::OnRuntimeStart()
	{
		mPhysicsWorld = new b2World({ 0.0f, -9.8f });

		auto view = mLevel->m_Registry.view<Rigidbody2DComponent>();
		for (auto e : view)
		{
			...
			{
				//XL_CORE_ASSERT(entity.HasComponent<CircleRendererComponent>());
	...
	void XLEngine::PhysicSystem2D::OnRuntimeStop()
	{
